...
if "form_reset" not in st.session_state:
    ...
    st.session_state.tipo_estud = None
    st.session_state.producto = None
    st.session_state.condicion = None
    st.session_state.material = None
    st.session_state.lote = ""
    st.session_state.cabina = ""
    st.session_state.protocolo = ""
    st.session_state.fecha_ingreso = date.today()
    st.session_state.tiempo_max = 3
...
with tab_form: 
        ...
        if st.session_state.reset_form:
            for key in ["tipo_analisis","cliente","tipo_estud","producto",
                        "condicion","material","lote","cabina",
                        "protocolo","fecha_ingreso","tiempo_max","meses"]:
        ...
        tipo_analisis = st.selectbox("Tipo de Análisis", tipos_analisis or ["(vacío)"], key = "tipo_analisis")
        cliente       = st.selectbox("Cliente", clientes or ["(vacío)"], key = "cliente")
        tipo_estud    = st.selectbox("Tipo de Estudio", tipos_estudio or ["(vacío)"], key = "tipo_estud")
        producto      = st.selectbox("Producto", productos or ["(vacío)"], key = "producto")
        condicion     = st.selectbox("Condición de almacenamiento", condiciones or ["(vacío)"], key = "condicion")
        material      = st.selectbox("Material de envase", materiales or ["(vacío)"], key = "material")
        lote          = st.text_input("Lote",  key = "lote")
        cabina        = st.text_input("Cabina", key = "cabina")
        protocolo     = st.text_input("Protocolo", key = "protocolo")
        fecha_ingreso = st.date_input("Fecha de Ingreso", value=date.today(), key = "fecha_ingreso")
        tiempo_max    = st.selectbox("Tiempo Máximo (meses)", [3,6,9,12,18,24,36,48], key = "tiempo_max")
        meses = st.multiselect("Puntos de Muestreo", list(range(0, tiempo_max+1)), key="meses")
            ...
            if st.button("💾 Guardar"):
                if not cliente or not producto or not lote:
                    ...
                    try:
                        ...
                        # Guardar mensaje en session_state
                        st.session_state["mensaje_exito"] = f"✅ Estudio ID {rid} creado correctamente."

                        st.session_state.reset_form = True   # ← aquí llamas la función
...
with tab_puntos:
    ...
    with get_conn() as con:
        df_puntos = pd.read_sql_query("""
            SELECT 
                id AS "ID",
                estudio_id AS "ID Estudio",
                mes AS "Mes",
                tipo_analisis AS "Tipo Análisis",
                cliente AS "Cliente",
                protocolo AS "Protocolo",
                tipo_estudio AS "Tipo Estudio",
                producto AS "Producto",
                lote AS "Lote",
                condicion AS "Condición",
                cabina AS "Cabina",
                material AS "Material Envase",
                fecha_ingreso AS "Fecha Ingreso",
                tiempo_max AS "Tiempo Máximo (meses)",
                fecha_vencimiento AS "Fecha Vencimiento",
                fecha_max_muestreo AS "Fecha Punto Muestreo"
