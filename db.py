    ...
    with get_conn() as con:
        ...
        parts = [
            ...
            "protocolo AS 'Protocolo'",
             "tipo_estudio AS 'Tipo de estudio'", 
             "producto AS 'Producto'", 
             "lote AS 'Lote'",
            sel(cols["condicion"], '"Condición de almacenamiento"'),
            "cabina AS 'Cabina'",
            sel(cols["material"], '"Material de envase"'),
            "fecha_ingreso AS 'Fecha de ingreso'",
            sel(cols["tiempo_max"], '"Tiempo máximo de muestreo"'),

            """
            CASE 
            WHEN fecha_ingreso IS NOT NULL AND tiempo_max IS NOT NULL
            THEN DATE(fecha_ingreso, '+' || tiempo_max || ' months')
            ELSE NULL
